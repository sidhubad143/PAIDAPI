...
RELEASEVERSION = "OB51"  # Updated to current version as of November 2025
...
async def create_jwt(uid: int, password: str) -> Tuple[str, str, str]:
    access_token, open_id = await getAccess_Token(str(uid), password)  # Ensure uid is str for payload
...
async def main():
    ...
    try:  
        print("\nGenerating JWT...")  
        token, lock_region, server_url = await create_jwt(int(uid), password)  # Convert uid to int as per function sig
        # return token  
